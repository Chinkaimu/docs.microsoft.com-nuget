...
description: 指导安装客户端工具，dotnet命令行工具、nuget 命令行工具、Visual Studio 的包管理.
author: Haic
ms.author: Haic
ms.date: 09/30/2022
ms.topic: quickstart
...
[MSBuild CLI](reference/msbuild-targets.md)也提供了恢复和创建包功能，在构建服务器方面非常有用。MSBuild 不是 NuGet 工作的通用工具。
...
.NET Core 2.0 CLI，dotnet.exe，适用于所有平台(Windows、 Mac 和 Linux) ，并提供核心 NuGet 特性，如安装、恢复和发布包。Dotnet 提供与.NET Core项目（例如.csproj）的直接集成，这在大多数情况下都很有帮助。Dotnet 也是直接为每个平台构建的，不需要您安装 [Mono](https://baike.baidu.com/item/mono/60423)。
